...
# 컬럼 설명 출처별 분류
schema_column_desc_from_schema = Gauge(
    'datahub_schema_column_desc_from_schema', 
    'Columns with description from schemaMetadata', 
    ['schema', 'platform']
)
schema_column_desc_from_editable = Gauge(
    'datahub_schema_column_desc_from_editable', 
    'Columns with description from editableSchemaMetadata', 
    ['schema', 'platform']
)
...
def get_datasets_by_platform_and_schema(platform, schema_name):
    ...
    schemaMetadata와 editableSchemaMetadata 모두 포함
    """
...
def analyze_schema_metrics(platform, schema_name):
    ...
    try:
        ...
        if not datasets:
            ...
            schema_column_desc_from_schema.labels(schema=schema_name, platform=platform).set(0)
            schema_column_desc_from_editable.labels(schema=schema_name, platform=platform).set(0)
        ...
        columns_desc_from_schema = 0
        columns_desc_from_editable = 0
        ...
        for dataset in datasets:
            ...
            # ===== 컬럼 설명 확인 (schemaMetadata + editableSchemaMetadata) =====
            column_descriptions = {}  # fieldPath -> (has_desc, source)


            # 1. schemaMetadata.fields에서 컬럼 수집
            ...
            if schema_metadata:
                ...
                if fields and isinstance(fields, list):
                    for field in fields:
                        ...
                        if has_desc:
                            columns_with_desc += 1
                            columns_desc_from_schema += 1
                            column_descriptions[field_path] = (True, 'schema')
                        else:
                            columns_without_desc += 1
                            column_descriptions[field_path] = (False, 'schema')


            # 2. editableSchemaMetadata.editableSchemaFieldInfo에서 추가 정보 조회
            editable_schema = dataset.get('editableSchemaMetadata')
            if editable_schema:
                editable_fields = editable_schema.get('editableSchemaFieldInfo')
                if editable_fields and isinstance(editable_fields, list):
                    for editable_field in editable_fields:
                        if editable_field is None:
                            continue
                        
                        field_path = editable_field.get('fieldPath')
                        if not field_path:
                            continue
                        
                        field_desc = editable_field.get('description')
                        has_desc = False
                        ...
                        # 이미 schemaMetadata에서 처리한 경우
                        if field_path in column_descriptions:
                            previous_has_desc, previous_source = column_descriptions[field_path]
                            
                            # editableSchemaMetadata에서 새로 설명이 추가된 경우
                            if has_desc and not previous_has_desc:
                                columns_with_desc += 1
                                columns_without_desc -= 1
                                columns_desc_from_editable += 1
                                column_descriptions[field_path] = (True, 'editable')
                        else:
                            # schemaMetadata에 없던 새로운 컬럼
                            total_columns += 1
                            if has_desc:
                                columns_with_desc += 1
                                columns_desc_from_editable += 1
                                ...
                                columns_without_desc += 1
                                column_descriptions[field_path] = (False, 'editable')
        ...
        # 컬럼 설명 출처별 분류
        schema_column_desc_from_schema.labels(schema=schema_name, platform=platform).set(columns_desc_from_schema)
        schema_column_desc_from_editable.labels(schema=schema_name, platform=platform).set(columns_desc_from_editable)
        ...
        log.info(f"✓ [{platform}/{schema_name}]   └─ From schemaMetadata: {columns_desc_from_schema}")
        log.info(f"✓ [{platform}/{schema_name}]   └─ From editableSchemaMetadata: {columns_desc_from_editable}")
